...
public class WebSecurityConfig {
    ...
    //1. 스프링 시큐리티 기능에서 제외 비활성화 ex) 정적요소들 (static 디렉토리)
    @Bean
    public WebSecurityCustomizer configure(){

        return (web) -> web.ignoring().requestMatchers(toH2Console()).requestMatchers("/static/**");
    }
    //requestMatchers : 특정 요청과 일치하는 URL에 대해 엑세스 설정함
    // permitAll() : 누구나 접근이 가능하게 설정
    //anyRequest() : 설정한 URL 이외에 요청에 대해 더설정하겠다.
    //authenticated() : 별도의 인가는 필요하지 않고 인증이 성공된 상태여야 접근이 가능하다.
    // loginPage("/login") : 로그인 페이지 경로 설정
    // .defaultSuccessUrl("/articles") : 로그인이 성공했을때 경로 설정
    // .logoutSuccessUrl("/login") : 로그아웃이 완료되었을때 이동할 경로 설정
    //.invalidateHttpSession(true) : 로그아웃 완료 후 해당 세션을 전체 삭제할지 여부 설정
    // .csrf().disable() : enable 하는게 좋지만 좀 힘들어서 실습 원활하게 하려고 비활성화 하는거임 ㅇㅇ
    ...
    //3. 인증 관리자 관련 설정  : 특정한 URL에 대한 엑세스 설정
    @Bean
